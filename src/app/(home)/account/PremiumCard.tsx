...
import { Crown, Sparkles, Zap, Shield, Infinity } from 'lucide-react'
...
export function PremiumCard({ isPremium }: PremiumCardProps) {
	...
	if (isPremium) {
		return (
			...
			>
				<div className='relative rounded-2xl bg-[#0e0e0e]/95 backdrop-blur-xl p-6'>
					<div className='absolute left-0 top-0 h-full w-[3px] bg-gradient-to-b from-yellow-400 via-yellow-400/60 to-transparent' />
					<div className='pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(250,204,21,0.12),transparent_70%)]' />
	...
	return (
		...
		>
			<div className='relative rounded-2xl bg-[#0e0e0e]/90 backdrop-blur-xl p-6 hover:bg-[#121212] transition-all duration-300'>
				...
				<div className='pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(234,179,8,0.08),transparent_70%)]' />
				...
				<div className='relative z-10 flex flex-col justify-between sm:flex-row sm:items-center gap-6'>
					<div className='flex items-center gap-4'>
						...
						<div>
							<div className='flex items-center gap-2'>
								<h2 className='text-lg font-bold text-white'>{t('title')}</h2>
								<span className='text-xs font-semibold px-2 py-0.5 rounded-full bg-white/10 text-white/50 border border-white/10'>
									{t('inactive')}
								</span>
							</div>
