	...
	if (isLoading || !userData) {
		return (
			<div className='w-full h-screen flex flex-col items-center justify-center gap-4 bg-gradient-to-br from-background via-muted/20 to-background text-foreground'>
				...
				<p className='text-sm text-muted-foreground'>{tLayout('loadingHint')}</p>
			</div>
		)
	}

	return (
		<div className='relative min-h-screen flex flex-col bg-gradient-to-br from-background via-black/[0.28] to-background text-foreground'>
			<div
			...
			<Button
				...
				className='fixed right-5 bottom-5 md:right-8 md:bottom-8 z-50 size-12 rounded-xl shadow-lg shadow-black/25 border border-border/50 bg-card/90 backdrop-blur-md hover:bg-card hover:scale-[1.03] transition-all p-0'
			>
